...
export default function Chat({ navigation }) {
  ...
  const [mensagens, setMensagens] = useState([
    { id: 1, texto: "Ola", tipo: "direita" },
    { id: 2, texto: "Oi", tipo: "esquerda" },
  ]);

  const enviarMensagem = () => {
    if (mensagem.trim() === "") return;

    setMensagens([...mensagens, { id: Date.now(), texto: mensagem, tipo: "direita" }]);
    setMensagem("");
  };
        ...
        <View style={styles.chatBox}>
          ...
          <ScrollView style={styles.mensagensChat}>
            {mensagens.map((msg) => (
              <View
                key={msg.id}
                style={msg.tipo === "direita" ? styles.msgDireita : styles.msgEsquerda}
              >
                <Text>{msg.texto}</Text>
              </View>
            ))}
